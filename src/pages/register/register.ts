...
export class RegisterPage {
  ...
  constructor (private viewCtrl: ViewController, private navCtrl: NavController, private loadingCtrl: LoadingController, private http: Http, private storage: Storage, private formBuilder: FormBuilder, private commonService: CommonServices) {
  ...
  userRegister (registerInfo) {
    if (this.registerForm.valid) {
      ...
      this.http.post(Keys.SERVICE_URL + '/register/registerUser', {headers: Keys.HEADERS}, {search: params}).subscribe((res) => {
        ...
        let result = res.json();
        if (result.code == '200') {
          this.commonService.showToast('注册成功', 'center');
          this.storage.set(Keys.USER_INFO_KEY, result.data);
          this.navCtrl.push(PersonalPage);
        }
        if (result.code == '201') {
          this.commonService.showToast('该手机号已注册或绑定', 'center');
        }
        if (result.code == '202') {
          this.commonService.showToast('服务器或网络异常, 请稍后再试', 'center');
        }
        if (result.code == '203') {
          this.commonService.showToast('验证码已过期', 'center');
        }
        if (result.code == '204') {
          this.commonService.showToast('然而验证码并不正确', 'center');
        }
      });
    }
  }
  ...
  getCodes (mobile) {
    ...
    this.http.post(Keys.USER_INFO_KEY + '/register/getMessageCode', {headers: Keys.HEADERS}, {search: params}).subscribe((res) => {
      let result = res.json();
      if (result.code == '200') {
        this.commonService.showToast('已发送', 'center');
