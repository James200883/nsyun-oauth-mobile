...
export class NickNamePage {
  ...
  constructor (public navCtrl: NavController, private loadingCtrl: LoadingController, private http: Http, private storage: Storage, private navParams: NavParams, private commonService: CommonServices) {
  ...
  saveNickname () {
    if (this.nickName.length < 2 || this.nickName.length > 10) {
      this.commonService.showToast('昵称:2-10个字符限制', 'center');
      return;
    }
